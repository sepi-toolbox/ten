...
const VERSION='0.46.0', BUILD='2026-09-02';
    ...
    if(asks.length&&c.kind!=='spell'){
      ...
      /* 이름이 없는 카드가 아홉 장 있다(투석기·분쇄기·삼지창·올빼미의 눈·영겁·해시계·
         ...
         비용이 달리는 식이다. 그래서 순서대로 이렇게 찾는다.
           ① 마지막 `키워드 — ` (분쇄기의 '파괴', 포식자의 '굴 파기' …)
              ⚠ 종류 이름(웨폰·실드…)은 키워드가 아니다 — 영겁이 그 함정이었다.
           ② 그것도 없으면 **마지막 문장** 앞에 붙인다(영겁·뇌엽절제기).
           ③ 문장이 하나뿐이면 결국 맨 앞이다(투석기). */
      if(!done&&pips){
        const kinds=Object.values(KINDKO);
        const re=/([^\s<>—.]{1,8}(?: [^\s<>—.]{1,8})?) — /g;
        let m,last=null;
        while((m=re.exec(body))){ if(kinds.indexOf(m[1])<0) last=m; }
        if(last){
          body=body.slice(0,last.index)+`<b>${last[1]}</b> ${pips}: `
