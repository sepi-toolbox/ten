...
const VERSION='0.37.0', BUILD='2026-09-02';
...
def('soulcatch',null,'유닛가 죽을 때마다 죽음 퀀텀 {N} 을 얻는다',(g,me,s)=>{ gain(me,2,s.up?3:2); });
...
def('v_thorn',null,'때린 유닛가 75% 확률로 독에 걸린다',(g,me,s,t)=>{
...
def('catapult','mycr','자기 유닛를 던진다. 체력이 클수록 큰 피해. 얼어 있으면 1.5배, 독은 상대에게 옮긴다',
  (g,me,s,t)=>{
    if(!t)return;
...
/* ⚠ 원문에 "Can not be destroyed or stolen" 이라 적힌 카드는 여기서 걸러진다.
   ...
   두 길**에서만 막는다. */
def('destroy','pm','대상 노드을 파괴한다',(g,me,s,t)=>{
  if(isFixed(t)){ log(`${t.c.ko} 은(는) 파괴되지 않는다`,'c'); return; }
  destroyPerm(t,me);
});
def('v_steal','pm','대상 노드을 빼앗는다',(g,me,s,t)=>{
...
def('appease','mycr','자기 유닛를 희생하고 +1|+1 을 얻는다',(g,me,s,t)=>{ kill(t); buff(s,1,1); });
def('devour','devour','체력이 자기보다 작은 유닛를 먹고 +1|+1. 유독한 몸을 먹으면 자기도 중독된다',
  (g,me,s,t)=>{
...
/* ⚠ 이건 **눌러서 쓰는 능력**이다(불의 정령·숲의 정령·중력자 불먹보 — 비용을 낸다).
   독수리·콘도르에서는 같은 id 가 **유닛가 죽을 때마다** 도는 쪽으로 붙는다.
   그래서 설명에 '언제' 를 적으면 한쪽이 반드시 거짓말이 된다 — '무엇을' 만 적고,
   '언제' 는 확대창의 머리말(능력 · N불 / 유닛가 죽을 때마다)이 말하게 둔다.
...
def('v_cold',null,'때린 유닛를 확률로 3턴 얼린다',(g,me,s,t)=>{ if(t&&Math.random()<0.3) freeze(t,3); });
...
/* ── 회복 ──────────────────────────────────────────────────────────────── */
def('mend','cr','대상 유닛를 5 회복시킨다',(g,me,s,t)=>{ healThing(t,s.up?10:5); });
...
def('v_hatch',null,'무작위 강화 유닛로 변한다',(g,me,s)=>{
...
def('fractal','cr','손패를 그 유닛로 채운다',(g,me,s,t)=>{
...
def('v_readiness','mycr','겨눈 유닛의 능력 비용이 0 이 된다. 시간 유닛는 이번 턴에 한 번 더 쓸 수 있다',
  (g,me,s,t)=>{
    if(!t)return;
    t.cast=0;
...
def('salvage',null,'노드이 부서질 때마다 +1|+1. 상대가 부순 노드은 턴에 한 번 사본이 손에 들어온다',
  (g,me,s,t,by)=>{
    if(!t)return;
    /* ⚠ '+1|+1' 은 신판 전용이다(`if is_open`). 원작판 조건은
       **내 턴이 아니고 · 부서진 노드이 내 것이 아닐 때 · 턴에 한 번**. */
    if(G.turn===me)return;
    if(t.own===me)return;
...
def('skull',null,'때린 유닛가 확률로 해골이 된다 (체력이 낮을수록 잘 걸린다)',(g,me,s,t)=>{
  if(!t||t.kind!=='creature')return;
...
def('v_freeevade',null,'내 문장이 바람이면, 날아다니는 내 유닛가 확률로 겨냥을 흘린다 (쌓을수록 확률이 오른다)',()=>{});
...
def('gaincharge',null,'유닛가 죽을 때마다 뼈가 {N} 늘어난다',(g,me,s,t,a)=>{ s.charges=(s.charges||0)+(a||2); });
...
/* ── 기타 지속 ─────────────────────────────────────────────────────────── */
def('nightfall',null,'모든 야행성 유닛가 +{N}|+1',()=>{});
...
def('stasis',null,'양쪽 유닛가 턴 끝에 공격하지 않는다 (웨폰는 그대로 때린다)',()=>{});
...
/* ⚠⚠ 원문: "Add X HP's to your maximum life points. X is the number of [대지] that you own."
   **유닛가 아니라 내 최대 체력**이다. 유닛에 +0|+N 을 주고 있었다 — 대상이 아예 틀렸다.
...
const EDGE=6;
def('flood',null,'매 턴, 여섯 칸째부터의 유닛를 죽인다. 물 속성 유닛는 무사하다',(g)=>{
  [g.me,g.ai].forEach(p=>{
    p.cr.forEach((u,i)=>{
      if(!u||i<EDGE)return;
...
/* ⚠⚠⚠ 인내의 파편 — 카드 글("+1/+0; 물 +2/+2, 범람하면 +4/+4")을 보고 **한 번 붙는
   고정 강화**로 만들어 뒀는데, openEtG 원작판(game.rs 공격 차례)은 전혀 다르다:
     매 턴 **내 유닛를 한 턴 묶고**(그래서 공격하지 않는다) **+2|+2 씩 쌓아 준다.**
     범람한 칸의 물 유닛는 **+5|+5**.
...
def('patience',null,'매 턴 내 유닛를 한 턴 묶고 +2|+2 를 쌓아 준다. 범람한 칸의 물 유닛는 +5|+5',()=>{});
...
def('v_freedom',null,'날아다니는 내 유닛가 확률로 피해 1.5배 · 실드 무시로 때린다 (쌓을수록 확률이 오른다)',()=>{});
...
def('paradox','paradox','공격력이 체력보다 큰 유닛를 죽인다',(g,me,s,t)=>{ if(t&&trueAtk(t)>t.hp) kill(t); });
...
def('undead',null,'시간 역행을 맞으면 무작위 유닛로 변한다',()=>{});
...
def('immolate','mycr','자기 유닛를 태운다. 12속성 퀀텀 1씩과 불 퀀텀 {N} 을 더 얻는다',(g,me,s,t,a)=>{
...
def('chimera',null,'자기 유닛를 하나로 합친다. 관성 · 중력 견인을 얻는다',(g,me)=>{
...
def('skyblitz',null,'바람 퀀텀을 모두 쓰고 비행 유닛가 급강하한다',(g,me)=>{
...
def('v_improve','cr','대상이 무작위 유닛로 바뀐다',(g,me,s,t)=>{
  if(!t)return;
...
def('swave','any','대상에게 액션 피해 4. 얼어붙은 유닛는 그대로 죽는다',(g,me,s,t)=>{
  if(t&&t.c&&t.frozen>0){ kill(t); return; }
  spellDmg(me,t,4);
});
def('enchant','pm','대상 노드이 실체를 잃는다 (겨눌 수 없다)',(g,me,s,t)=>{
  if(t&&!t.flags.includes('immaterial')) t.flags.push('immaterial');
});
def('v_rewind','cr','대상 유닛를 주인의 덱 맨 위로 되돌린다',(g,me,s,t)=>{
  if(!t||t.kind!=='creature')return;
  const p=t.own;
  /* 원작의 특수 처리 — 해골은 아무 유닛로, 미이라는 파라오로 되돌아간다 */
  let code=t.c.code;
...
def('accretion','pm','대상 노드을 부수고 +0|+15. 체력이 45를 넘으면 스스로 무너져 블랙홀이 손에 들어온다',
  (g,me,s,t)=>{
...
/* ⚠ 해체공(salvage)은 **노드이 부서지는 순간**에 걸려 있다. 그 사건이 아예 없어서
...
function shieldReduce(sh,atk,dmg){
  ...
  /* ⚠ 원문: "reduced by **1 + N**". 1 을 빼먹어 빛 유닛가 없으면 아무것도 안 막았다. */
...
function playCard(p,u,tgt){
  /* ⚠⚠ **대상이 필요한 카드를 대상 없이 내면 안 된다.** 막지 않으면 능력 함수가
     null 을 받아 그 자리에서 판이 죽는다(정화가 겨눌 유닛가 없을 때 실제로 그랬다).
     한 판에 유닛가 하나도 없을 때 상대가 그 액션을 뽑으면 바로 터진다. */
...
/* ⚠⚠ 속성·종류는 **일러스트 영역에 얹지 않는다.** 그림 자리는 그림 자리다.
   ...
   이미 있는 **발치 줄** 가운데의 빈 자리를 쓴다. 유닛는 좌우에 공/체가 있고
   가운데가 비어 있고, 액션·노드은 줄 전체가 비어 있다. */
function typeTag(c){
...
function packRow(el){
  ...
  /* 웨폰·실드 자리는 양 끝에 못박혀 있다 — 노드이 그 위로 올라가면 안 된다 */
...
function zoomHTML(c,u){
  ...
  sks.forEach(s=>{ const h=SK[s.id]; if(!h)return;
    ...
    const EVKO={cast:'', ownattack:'매 턴', hit:'때릴 때', death:'유닛가 죽을 때마다',
      owndeath:'이 몸이 죽을 때', shield:'맞을 때', ownplay:'낼 때', postauto:'공격을 마치고',
      attack:'공격할 때', buff:'지속', ownbuff:'지속', destroy:'노드이 부서질 때',
      prespell:'액션을 맞을 때', discard:'버려질 때', owndiscard:'손에서 버려질 때',
      ownhp:'체력 계산', hp:'체력 계산'};
    const nm=abilName(c,s);
    const when=s.ev==='cast'?(c.kind==='spell'?'액션 효과':`${cst?cst+' '+ELKO[ce]:'무료'}`)
      :(EVKO[s.ev]||'지속');
