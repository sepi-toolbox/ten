...
const VERSION='0.27.0', BUILD='2026-09-02';
...
def('v_virusplague',null,'자신을 희생해 상대 크리처 전부에게 독을 준다',(g,me,s)=>{
...
/* ⚠ 원문: "attacking creatures have a 75% chance to be **infected**" — 피해는 없다.
   피해 2 를 얹어 두는 바람에 원작보다 훨씬 센 방패가 돼 있었다. */
def('v_thorn',null,'때린 크리처가 75% 확률로 독에 걸린다',(g,me,s,t)=>{
...
/* ⚠ 원문에 "Can not be destroyed or stolen" 이라 적힌 카드는 여기서 걸러진다.
   기물 스스로 사라지는 길(유지 비용·희생)까지 막으면 안 되므로 **바깥에서 손대는
   두 길**에서만 막는다. */
def('destroy','pm','대상 기물을 파괴한다',(g,me,s,t)=>{
  if(isFixed(t)){ log(`${t.c.ko} 은(는) 파괴되지 않는다`,'c'); return; }
  destroyPerm(t,me);
});
def('v_steal','pm','대상 기물을 빼앗는다',(g,me,s,t)=>{
...
def('momentum','cr','대상이 방패를 무시하고 +1|+1 을 얻는다',(g,me,s,t)=>{
...
def('v_hope',null,'받는 피해를 1 깎고, 빛을 내는 내 크리처 수만큼 더 깎는다',()=>{});
...
/* ⚠ 원문은 "random **elite** creature" — 강화판으로 부화한다. */
def('v_hatch',null,'무작위 강화 크리처로 변한다',(g,me,s)=>{
  const base=ETG.cards.filter(c=>c.kind==='creature'&&!c.up&&playable(c)&&c.upcode);
  const c=base[rnd(base.length)];
  morph(s,CARD[c.upcode]?c.upcode:c.code);
});
...
def('v_nymph','mypillar','겨눈 내 기둥·진자를 그 속성의 님프로 바꾼다',(g,me,s,t)=>{
  ...
  if(!u){ me.pm[i]=t; return; }                 /* 크리처 칸이 없으면 기둥은 그대로 둔다 */
  log(`${t.c.ko} → ${u.c.ko}`,'a');
});
...
def('boneyard',null,'해골이 아닌 크리처가 죽을 때마다 해골이 나온다',(g,me,s,t)=>{
...
/* ⚠⚠ 원문: "Gain the target weapon's **ability** and +X|+2. X is the weapon's attack."
   ① 겨누는 것은 판 위의 무기다(상대 무기로 굳혀 놨었다)
   ② **능력을 통째로 베낀다** — 이게 성전사의 본체인데 통째로 빠져 있었다
   ③ 체력도 +2 붙는다 */
def('v_endow','weapon','겨눈 무기의 능력과 +X|+2 를 얻는다 (X 는 그 무기의 공격력)',(g,me,s,t)=>{
  if(!t)return;
  buff(s,t.atk,2);
...
def('v_readiness','mycr','겨눈 크리처의 능력 비용이 0 이 된다. 시간 크리처는 이번 턴에 한 번 더 쓸 수 있다',
  (g,me,s,t)=>{
    if(!t)return;
    t.cast=0;
...
/* ── 방패 ──────────────────────────────────────────────────────────────── */
/* ⚠ 같은 id 가 두 카드에 붙어 있다 —
     소산 방패(shield 이벤트) = 엔트로피 1로 피해 3을 막는다  … shieldReduce 가 처리
     세라핌(cast 이벤트)      = **한 턴 동안 겨눌 수 없다**   … 여기서 처리
   세라핌 쪽 설명이 방패 글로 나와 있어서, 눌러도 방패가 생기는 줄 알았다. */
...
/* ⚠ openEtG 에서 `Self::v_noluci => ()` — **아무 일도 하지 않는 표식**이다(단검류 소검·장검).
   원작 카드 글에도 "Deal 3 damages" 밖에 없다. '방패를 무시한다' 는 내가 지어낸 말이었다.
   설명이 비면 ruleText 가 알아서 뺀다. */
def('v_noluci',null,'',()=>{});

/* ── 기타 지속 ─────────────────────────────────────────────────────────── */
def('nightfall',null,'모든 야행성 크리처가 +{N}|+1',()=>{});
...
def('undead',null,'시간 역행을 맞으면 무작위 크리처로 변한다',()=>{});
...
/* ⚠⚠ 원문(양쪽 다): "Gain {N}:6 plus 1 quanta of **each other element**."
   크리처 체력을 불 퀀텀에 더하고 있었다 — 표에도 원문에도 없는 값이다.
   12속성 1씩 + 불 {N} → 불은 합쳐서 {N}+1 (분신 6, 화장 8). 원문의 "6 [불]" 과 맞는다. */
def('immolate','mycr','자기 크리처를 태운다. 12속성 퀀텀 1씩과 불 퀀텀 {N} 을 더 얻는다',(g,me,s,t,a)=>{
...
def('dryspell',null,'모든 크리처에게 피해 1. 준 피해만큼 물 퀀텀을 얻는다',(g,me)=>{
  let n=0; all().forEach(u=>{ if(u&&u.kind==='creature') n+=spellDmg(me,u,1); });
  gain(me,7,n);
});
/* 건조(강화) — 상대 크리처에게 2, 준 만큼 물 */
def('v_dessication',null,'상대 크리처에게 피해 2. 준 피해만큼 물 퀀텀을 얻는다',(g,me)=>{
...
def('wisdom','any','대상에게 +4|+0. 실체 없는 대상은 염동을 얻어 방패를 무시한다',(g,me,s,t)=>{
  if(!t||!t.c)return;
  buff(t,4,0);
...
function spellDmg(me,t,n){
  ...
  /* ⚠ 반사 방패·에메랄드 방패 — 원문: "Any damage from spells is reflected."
     적혀 있는데 아무 데도 없었다. 주문 피해가 얼굴로 갈 때만 되돌린다. */
...
function dmgThing(u,n){
  ...
  /* ⚠ 부두 인형 — 원문: "damage and status is inflicted to your opponent as well".
     ...
     한 통로에서 처리한다 — 그래야 주문이든 공격이든 빠짐없이 돈다. */
...
/* ⚠ 해체공(salvage)은 **기물이 부서지는 순간**에 걸려 있다. 그 사건이 아예 없어서
...
/* 무기의 문장 보너스 — openEtG: bow=1:8 1:9(강화)/1:9, hammer=1:3 1:4, v_dagger=1:2 1:11 */
...
/* ⚠ 원문: "Creatures do not attack at the end of **each player's** turn" —
   해시계는 양쪽 크리처를 다 멈춘다(무기는 멈추지 않는다). */
...
function ruleText(c,u,full){
  ...
  /* ⚠ 평범한 방패는 능력이 없고 감소량이 체력칸에 숫자로만 있었다 —
     설명란이 통째로 비어 '아무 일도 안 하는 카드' 처럼 보였다. 글로도 적어 준다. */
  /* ⚠ 에메랄드 방패처럼 표식이 붙은 방패는 이 줄이 통째로 빠져 있었다
...
/* ⚠⚠ 속성·종류는 **일러스트 영역에 얹지 않는다.** 그림 자리는 그림 자리다.
   ...
   이미 있는 **발치 줄** 가운데의 빈 자리를 쓴다. 크리처는 좌우에 공/체가 있고
   가운데가 비어 있고, 주문·기물은 줄 전체가 비어 있다. */
function typeTag(c){
...
function packRow(el){
  ...
  /* 무기·방패 자리는 양 끝에 못박혀 있다 — 기물이 그 위로 올라가면 안 된다 */
...
function zoomHTML(c,u){
  ...
  /* ⚠⚠ **지금 이 몸에 걸린 것 전부.** 예전에는 얼음·지연·독·사용·쌓임 다섯뿐이라,
     중력 견인·아드레날린·급강하·굴속·증기·아플라톡신·신성한 방패처럼 **효과로 붙은 상태**가
  ...
  if(u){
    ...
    if(u.dshield) st.push('신성한 방패 — 다음 내 턴까지 겨눌 수 없다');
...
/* ⚠⚠ 위의 CSS 로도 안 잡히는 경우가 남는다(핀치 확대 뒤의 패닝, 잠깐 뜬 키보드 등).
   그래서 **스스로 되돌린다** — 전투 중에 문서가 조금이라도 밀리면 즉시 0 으로 돌린다.
   덱 화면과 에디터는 스크롤해야 하므로 건드리지 않는다.
   ⚠ `scroll` 은 아주 잦게 오므로 여기서는 값 비교만 하고 아무것도 다시 그리지 않는다. */
function snapTop(){
  /* ⚠⚠ 카드 에디터도 이 엔진 파일을 쓴다 — 거기서 이걸 돌리면 **목록이 스크롤이 안 된다.**
     같은 코드를 두 페이지가 쓰는 이상, '이 페이지가 판인가' 를 반드시 물어야 한다. */
  if(window.ETG_NOBOOT)return;
  if(document.body.classList.contains('deckmode'))return;
  const de=document.scrollingElement||document.documentElement;
  if(de.scrollTop||window.scrollY||document.body.scrollTop){ window.scrollTo(0,0); }
}
addEventListener('scroll',snapTop,{passive:true});
addEventListener('orientationchange',()=>setTimeout(snapTop,120));
...
window.ETGDBG={get G(){return G;},SK,MISS,POOLS,CARD,BYNAME,playable,startGame,autoDeck,
  ...
  showZoom,etgCardHTML,VERSION,playPerm,syncAuras,spellDmg,destroyPerm,ruleText,trueAtk,
  MOD,get MODREF(){return MOD;},setMod(m){MOD=m;},saveMod,applyMod,modded,ORIG,
  ELKO,FLAGKO,ELC,MODKEY,esc};
