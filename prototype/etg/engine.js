...
def('snipe','cr','대상 크리처에게 피해 3',(g,me,s,t)=>{ spellDmg(me,t,3); });
...
def('virusinfect','cr','자신을 희생해 겨눈 크리처에게 독 1을 준다',(g,me,s,t)=>{
...
def('devour','devour','체력이 자기보다 작은 크리처를 먹고 +1|+1. 유독한 몸을 먹으면 자기도 중독된다',
  (g,me,s,t)=>{
...
def('fractal','cr','손패를 그 크리처로 채운다',(g,me,s,t)=>{
...
def('paradox','paradox','공격력이 체력보다 큰 크리처를 죽인다',(g,me,s,t)=>{ if(t&&trueAtk(t)>t.hp) kill(t); });
...
def('butterfly','butterfly','대상의 능력이 「3엔트로피 : 기물 파괴」 로 바뀐다',(g,me,s,t)=>{
...
def('v_improve','cr','대상이 무작위 크리처로 바뀐다',(g,me,s,t)=>{
  if(!t)return;
...
def('v_rewind','cr','대상 크리처를 주인의 덱 맨 위로 되돌린다',(g,me,s,t)=>{
  if(!t||t.kind!=='creature')return;
  const p=t.own;
...
function ruleText(c,u,full){
  ...
  if(c.txt) return esc(c.txt);                 /* 개조로 직접 적어 둔 글 */
  /* ⚠⚠ 카드에 실리는 글은 **인쇄된 원문을 문장 그대로 옮긴 한국어**(kotxt)다.
     예전에는 능력 하나하나의 설명을 ' · ' 로 이어 붙여 만들었는데, 그건 원문이 아니라
     내가 쓴 요약이라 문장이 조각나고 무기의 첫 줄('무기 — 매 턴 피해를 준다') 같은
  ...
  if(c.kotxt){
    ...
    let body=fillCard(c,esc(c.kotxt));
    /* 앞에 세운 표식을 글이 또 부르면 한 번만 — '관성 · 관성 — 방패를…' 이 되지 않게 */
