...
FIXED = {
    ...
    # 겁화룡 이그니스 — 보스. **홍염 넷을 깔고 시작**하고(개전), 이그니스가 2막이다.
    #   HP 20 이하로 떨어져야만 이그니스를 낼 수 있다 → 그때부터 얼굴이 안 맞는 대신
    #   20/20 수호를 뚫어야 이긴다. 그 몸이 죽으면 그 자리에서 보스가 진다.
    # ⚠ AI 는 이그니스를 **절대 제물로 쓰지 않는다**(sacOK) — 깃털·도화선·분신·전차 전부.
    # TODO 단계별 새 카드는 아직 안 정했다 — 지금은 세 단계가 같은 덱이다.
    "fire_ignis": {
        "cards": {
            0: [
                ("홍염", 4),                                            # 개전 — 시작부터 판에 넷
                ("도화선", 2), ("불사조의 깃털", 2), ("작열 좀비", 2),     # 2코 6
                ("용의 숨결", 3),                                       # 3코 3
                ("불의 군단", 3),                                       # 4코 3
                ("불사조", 2),                                          # 5코 2
                ("용암거인", 3), ("이그니스", 2),                         # 8코 5
