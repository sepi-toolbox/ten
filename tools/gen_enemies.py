...
FIXED = {
    ...
    # ⚠ 고블린 카드는 11종뿐이라 **플레이어 골격(20종 · 커브 1코3~6코2)을 따르지 않는다.**
    #    적 덱은 덱 구성 규칙이 아니라 '이 적이 무엇을 하는가' 를 따른다 —
    #    커브가 4코에서 끝나는 것이 곧 이 적의 성격이다(속공 아키타입).
    # ⚠ 23장을 채우려면 한 종은 3장이 필요하다. 얼굴 마담인 고블린 폭탄병에 준다
    #    (동명 2장 상한의 예외 — 기본 지형 17장과 같은 취급).
    "fire_goblin": {
        "cards": [
            ("고블린 폭탄병", 3), ("파이어 볼트", 2),                        # 1코 5
            ("용암 정령", 2), ("도화선", 2), ("불사조의 깃털", 2),             # 2코 6
            ("고블린 지휘관", 2), ("고블린 전차", 2),
            ("불꽃광대", 2), ("고블린 화염포", 2),                          # 3코 8
            ("고블린 방패병", 2), ("와이번", 2),                            # 4코 4
        ],
    ...
    #   용암 정령(3/1)은 도화선·연소로 그 턴에 죽는 자폭병이고, 불씨 살리기가 그 시계를 되돌린다.
    #   정령의 불꽃은 남은 패를 통째로 3/1 자폭병으로 바꾸는 마무리다 — 손이 마르면 그때 쓴다.
    "fire_arson": {
        "cards": [
            ("불씨정령", 3), ("파이어 볼트", 3),                        # 1코 6
            ("용암 정령", 3), ("도화선", 3), ("불사조의 깃털", 2),        # 2코 8
            ("불씨 살리기", 3),                                      # 3코 3
            ("와이번", 3), ("정령의 불꽃", 3),                         # 4코 6
        ],
        "lands": [("화산", 17)],
    },
    # 화염술사 카린 — **주문 덱**. 크리처는 화염 아귀 하나로 몰고, 나머지는 전부 불을 쏜다.
    #   화염 아귀는 주문 한 장당 HP +1 이라 화염 방패(+0/+5)를 받으면 파이어볼을 맞고도 산다.
    #   파이어볼이 아군까지 태우는 것은 실수가 아니라 이 적의 성격이다 — 판을 비우고 시작한다.
    "fire_karin": {
        "cards": [
            ("불씨정령", 3), ("파이어 볼트", 4), ("고블린 폭탄병", 2),    # 1코 9
            ("화염 방패", 3), ("불사조의 깃털", 3),                     # 2코 6
            ("화염 아귀", 4), ("파이어볼", 4),                         # 3코 8
        ],
        "lands": [("화산", 17)],
    },
