...
(async()=>{
  ...
  /* ⚠ 계곡이 뽑는 몸 = 덱 카드 파이어버그와 **같은 카드**다. gen_decks 에서 능력치를
     고치면 여기도 같이 바뀐다(3/1 → 1/1 → 2/1 로 두 번 움직였다). */
  ok('계곡 — 파이어버그', fire.계곡몸==='파이어버그 2/1 폭발 | 파이어버그 2/1 폭발', fire.계곡몸);
  ...
  /* ⚠ 원정에는 **동명 카드 제한이 없다**(2026-08). 이미 가진 지형도 또 나온다 —
     화염의 원천 둘을 쌓는 것도 플레이어의 선택이다. */
  ok('이미 가진 지형도 또 나온다', off.중복===true, `화염의 원천 재등장 ${off.중복}`);
