...
GLOSSARY = {
    ...
    # ── 어둠 ──
    # ⚠ 2026-08 '대가' → **제물** 개명. 불 덱에 같은 규칙의 태그를 또 만들 뻔해서 하나로 합쳤다.
    "제물":   ["어둠", "소환할 때 내 HP를 적힌 만큼 지불한다. HP 8 = 마나 1로 환산해 그만큼 강하다."],
    "연격":   ["불", "한 턴에 적힌 횟수만큼 공격한다. 수호 한 장에 통째로 막히는 것이 약점이다."],
    "광분":   ["불", "이 개체의 HP가 **줄 때마다** 상대 플레이어를 자기 ATK 만큼 한 번 더 친다. 가호·면역으로 실피해가 0이면 켜지지 않는다."],
    # ⚠ 2026-08 회복 대상이 **본체에서 그 크리처 자신으로** 바뀌었다.
    "흡혈":   ["어둠", "이 개체가 공격해 입힌 피해의 **절반**만큼 **자기 HP**를 회복한다(반올림, 최대 체력까지). 막히면 회복도 없다."],
    # ── 빛 ──
    "가호":   ["빛", "처음 받는 피해를 통째로 무효로 한다. 단 한 번뿐이고, 유지되는 동안 노랗게 빛난다."],
    "축복":   ["빛", "소환할 때 내 HP를 적힌 만큼 회복한다."],
    # ── 어둠 ──
    "단말마": ["어둠", "이 개체가 **소멸할 때** 뒤에 적힌 효과가 발동한다. "
              "'(1회)' 가 붙으면 되살아난 몸은 그 능력을 잃는다."],
    "출진":   ["어둠", "이 카드를 **소환할 때** 뒤에 적힌 효과가 발동한다. "
              "덱에서 스스로 터지는 '개전' 과는 다르다 — 손에서 내야 발동한다."],
    # ── 기타 ──
