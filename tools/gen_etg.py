...
# ⚠ 강화판(up) 이름은 여기 넣지 않는다 — 성권: "강화판 자체가 없을거니까 지워
#   데이터상으로만 남겨". 데이터에는 남지만 게임 어디에도 안 나오므로 이름이 필요 없다.
SEPI = {
...
def main():
    ...
    # ── 강화판 이름은 **기본 카드와 같게** ────────────────────────────────
    # 성권: "강화판 자체가 없을거니까 지워 데이터상으로만 남겨" →
    #       "그럼 그냥 동명의 카드로 나오게 해 이름은"
    # ⚠ 강화판은 게임 어디에도 안 나온다(덱·에디터·확대창 전부에서 뺐다). 그런데도
    #   '장단검·자수정 탑·정예 해골' 같은 **딴 이름**을 데이터에 들고 있으면, 나중에
    #   그 이름이 어디선가 새어 나왔을 때 "이런 카드가 있었나" 하고 헷갈리게 된다.
    #   같은 카드의 다른 수치일 뿐이므로 이름도 같게 둔다.
    # ⚠ 영문 이름(en)은 원작 그대로 남긴다 — 원문 대조의 기준이라 건드리면 안 된다.
    for c in cards:
        if c["up"]:
            base = idx.get(c["code"] - 2000)
            if base:
                c["ko"] = base["ko"]
