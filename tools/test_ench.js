...
(async()=>{
  ...
  const R=await p.evaluate(async()=>{
    ...
    reset(); put('ai','창병'); put('me','검사'); resolveOnFoe('me','소이탄',0); o.소이탄=hp('me',0);
  ...
  ok('HP 지불 스펠', R.피의못===8&&R.금단===12, `피의 못 −${R.피의못} · 금단의 지식 −${R.금단}`);
  ok('요격 뒤 조항', R.심판===8&&R.소이탄===2&&R.포식[0]===4,
     `심판 +${R.심판} 회복 · 소이탄 자해 후 검사 ${R.소이탄} · 포식 버프 ${R.포식.join('/')}`);
